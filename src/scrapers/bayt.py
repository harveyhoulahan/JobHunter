"""
Remotive MENA scraper (replaces Cloudflare-blocked Bayt.com).
Uses Remotive public API filtered to Middle East / worldwide remote roles.
Complements zerotaxjobs.py with a distinct source label so dedup works correctly.
...
# Bayt focuses on product / qa categories to avoid identical results to zerotaxjobs
_CATEGORIES = ["product", "qa", "customer-support"]


class BaytScraper(BaseScraper):
    """Scraper for Remotive remote jobs open to the MENA region (product/QA focus)."""
    ...
    def search_jobs(self, search_terms: List[str], location: str = "Middle East / Remote") -> List[Dict[str, Any]]:
        ...
        for cat in _CATEGORIES:
            try:
                ...
                logger.warning(f"Remotive MENA/Bayt ({cat}): {exc}")

        logger.info(f"Remotive MENA (bayt): {len(jobs)} jobs")
        return jobs
    ...
    def _parse(self, item: dict) -> Dict[str, Any]:
        try:
            ...
            # prefix source to make source_id distinct from zerotaxjobs (same Remotive IDs)
            source_id = hashlib.md5(f"bayt_{url}".encode()).hexdigest()[:12]
            ...
            logger.debug(f"Remotive MENA/Bayt parse error: {exc}")
            return {}
