"""
Kimi-Powered Cover Letter Generator
Uses Moonshot Kimi to create compelling, highly personalized cover letters
...
class GPT4CoverLetterGenerator:
    """
    Cover letter generation using Kimi (Moonshot AI).
    Kept the class name for compatibility.
    """
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        logger.info("Kimi Cover Letter Generator initialized")
...
FORBIDDEN:
- Do NOT use: "actionable"drawn to", "aligns", "honed", "spearheaded", "robust", "leveraging", "passionate", "innovative", "cutting-edge"
...
"""
    ...
    def _build_prompt(
        ...
        edu = edu_raw if isinstance(edu_raw, dict) else (edu_raw[0] if edu_raw else {})
        skills = HARVEY_PROFILE.get('skills', {})
        # Support both old flat keys (ai_ml/backend/…) and current tier keys (core/strong/familiar)
        def _skill_list(keys, n):
            for k in keys:
                v = skills.get(k)
                if v:
                    return list(v)[:n]
            return []
        ai_skills      = _skill_list(['ai_ml', 'core'], 8)
        backend_skills = _skill_list(['backend', 'strong'], 8)
        fs_skills      = _skill_list(['fullstack', 'core'], 6)
        cloud_skills   = _skill_list(['cloud', 'familiar'], 4)
        data_skills    = _skill_list(['data', 'strong'], 4)
        profile_summary = f"""
CANDIDATE: Harvey Houlahan
EDUCATION: {edu.get('degree','B.S. Advanced Computer Science')} from {edu.get('university') or edu.get('institution','Monash University')}
LOCATION: {HARVEY_PROFILE.get('location', {}).get('base','Byron Bay, NSW') if isinstance(HARVEY_PROFILE.get('location'), dict) else HARVEY_PROFILE.get('location','Byron Bay, NSW')}

BACKGROUND:
{HARVEY_PROFILE.get('summary','')}

CORE TECHNICAL SKILLS:
- AI/ML: {', '.join(ai_skills)}
- Backend: {', '.join(backend_skills)}
- Full-stack: {', '.join(fs_skills)}
- Cloud & Data: {', '.join(cloud_skills)}, {', '.join(data_skills)}
"""
